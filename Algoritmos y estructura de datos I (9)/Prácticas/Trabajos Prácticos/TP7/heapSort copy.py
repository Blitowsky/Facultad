...
def delMax(BH):
    ...
    if(length(BH)<1):
        ...
        if(length(BH)==1):
            retval = BH.bheaplist.head.nextNode
            linkedlist.delete(BH.bheaplist,1)
        else:
            retval = BH.bheaplist.head.nextNode
            exchange = linkedlist.access(BH.bheaplist,length(BH)-1)
            linkedlist.delete(BH.bheaplist,length(BH)-1)
            linkedlist.update(BH.bheaplist,exchange,1)
            shiftDown(BH,1)
...
def shiftUp(BH,i):
    ...
    while i // 2 > 0:
        ...
        if(linkedlist.access(BH.bheaplist,i) > linkedlist.access(BH.bheaplist,i//2)):
            exchange = linkedlist.access(BH.bheaplist,i//2)
            linkedlist.update(BH.bheaplist,linkedlist.access(BH.bheaplist, i),i//2)
            linkedlist.update(BH.bheaplist,exchange,i)
            i= i//2
        else:
          break

#SHIFTDOWN PARA EL MAX HIP, EL PADRE ES MAS GRANDE QUE EL HIJO.
def shiftDown(BH,i,currentsize=None):
    ...
    while (i * 2) <= currentsize:
        ...
        #
        if(linkedlist.access(BH.bheaplist, mc)>linkedlist.access(BH.bheaplist,i)):
            exchange = linkedlist.access(BH.bheaplist,i)
            linkedlist.update(BH.bheaplist,linkedlist.access(BH.bheaplist, mc),i)
            linkedlist.update(BH.bheaplist,exchange,mc)
            i = mc
        else:
            break
        #
