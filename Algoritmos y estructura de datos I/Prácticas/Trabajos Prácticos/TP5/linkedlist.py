...
def deleteByIndex(L,index):
    if nodeByIndex(L,index) == None:
        return None
    else:

        index_node = nodeByIndex(L,index)

        if nodeByIndex(L,index-1) != None :

            if index > 0: nodeByIndex(L,index-1).value
            if index < 0: nodeByIndex(L,index+1).value

            nodeByIndex(L,index-1).nextNode = nodeByIndex(L,index+1)

            return index_node
        elif index == 0:
            L.head = nodeByIndex(L,index+1)
            return index_node
        else:
            return None
